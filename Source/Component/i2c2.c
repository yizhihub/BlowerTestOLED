...
//      #include "hard_ware.h"
    ...
    #define IIC2_SCL         PDout(3)                                    /* 精英主板V.7  CAMERA接口*/ 
    #define IIC2_SDA         PGout(13)
    #define IIC2_SDA_R       PGin(13)
    #define IIC2_SCL_R       PDin(3)
    #define IIC2_SDA_IN      {GPIOG->CRH &= ~(0xFu << (13 - 8) * 4); GPIOG->CRH |= 8u << (13 - 8) * 4;}
    #define IIC2_SDA_OUT     {GPIOG->CRH &= ~(0xFu << (13 - 8) * 4); GPIOG->CRH |= 3u << (13 - 8) * 4;}
    #define IIC2_SCL_IN      {GPIOD->CRL &= ~(0xFu << (3) * 4); GPIOD->CRL |= 8u << (3) * 4;}
    #define IIC2_SCL_OUT     {GPIOD->CRL &= ~(0xFu << (3) * 4); GPIOD->CRL |= 3u << (3) * 4;}
    
//    #define IIC2_SCL         PBout(11)                                   /* 精英主板V.7  ATK MODULE接口*/
//    #define IIC2_SDA         PBout(10)
//    #define IIC2_SDA_R       PBin(10)
//    #define IIC2_SCL_R       PBin(11)
//    #define IIC2_SDA_IN      {GPIOB->CRH &= ~(0xFu << (10 - 8) * 4); GPIOB->CRH |= 8u << (10 - 8) * 4;}
//    #define IIC2_SDA_OUT     {GPIOB->CRH &= ~(0xFu << (10 - 8) * 4); GPIOB->CRH |= 3u << (10 - 8) * 4;}
//    #define IIC2_SCL_IN      {GPIOB->CRH &= ~(0xFu << (11 - 8) * 4); GPIOB->CRH |= 8u << (11 - 8) * 4;}
//    #define IIC2_SCL_OUT     {GPIOB->CRH &= ~(0xFu << (11 - 8) * 4); GPIOB->CRH |= 3u << (11 - 8) * 4;}
#elif defined(STM32F10X_MD)
    #define IIC2_SCL          PBout(6)
    #define IIC2_SDA          PBout(7)
    #define IIC2_SDA_R       PBin(7)
    #define IIC2_SDA_IN      {GPIOB->CRL &= 0X0FFFFFFF; GPIOB->CRL |= 8 << 28;}
    #define IIC2_SDA_OUT     {GPIOB->CRL &= 0X0FFFFFFF; GPIOB->CRL |= 3 << 28;}
#elif defined(GD32F303RE)
    #define IIC2_SCL         PBout(6)                                    /* 风机测试平台 */ 
    #define IIC2_SDA         PBout(7)
    #define IIC2_SCL_R       PBin(6)
    #define IIC2_SDA_R       PBin(7)
    #define IIC2_SCL_IN      {GPIOB->CRL &= ~(0xFu << (6) * 4); GPIOB->CRL |= 8u << (6) * 4;}
    #define IIC2_SCL_OUT     {GPIOB->CRL &= ~(0xFu << (6) * 4); GPIOB->CRL |= 3u << (6) * 4;}
    #define IIC2_SDA_IN      {GPIOB->CRL &= ~(0xFu << (7) * 4); GPIOB->CRL |= 8u << (7) * 4;}
    #define IIC2_SDA_OUT     {GPIOB->CRL &= ~(0xFu << (7) * 4); GPIOB->CRL |= 3u << (7) * 4;}


#else
    sbit IIC2_SDA = P1^0;
    sbit IIC2_SCL = P1^1;
    #define IIC2_SDA_R   IIC2_SDA
    #define IIC2_SDA_IN
    #define IIC2_SDA_OUT
#endif

#define IIC2_SCL_0    IIC2_SCL = 0
#define IIC2_SCL_1    IIC2_SCL = 1
#define IIC2_SDA_0    IIC2_SDA = 0
#define IIC2_SDA_1    IIC2_SDA = 1
...
static void i2cStart(void)
{
    ...
    IIC2_SDA_0;
    usDelayOne();
    IIC2_SCL_0;                                                /* there might be a delay after pull scl down */
}
...
static uint8_t i2cRespons()
{
     ...
     * 3. reduce a usDelayOne() after IIC2_SDA_1.
     */ 
    IIC2_SDA_IN;                                                        /* master handover sda's control  */
    IIC2_SDA_1;
    usDelayOne();
    IIC2_SCL_1;
     ...
     */
    while(IIC2_SDA_R && (ucTimeOut < 5u)) {
        ucTimeOut++;
        usDelayOne(); 
    }
    IIC2_SCL_0;                                                         /* after this, slave release sda */
    IIC2_SDA_OUT;                                                       /* maseter recontrol sda         */
...
//    IIC2_SCL_0; // is it indispensable ?
    IIC2_SDA_IN;
    IIC2_SDA_1;
