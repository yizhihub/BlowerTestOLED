...
static uchar GnProtocalSelect = 0;
u8 VerSionStr[24] = {0};
...
uchar UartCMDSpeedTxBuf1[10] = {0x2B, 0xC1, 0x3F};
...
{
    KEYn_e eKeyPress;
    uint8_t ii = 0, ul10msCnt = 0, bFlip = 0, bErDispFlg = 0, nProSelCnt = 0;
    INT16U  usVol;
    INT16S  sCur;
...
//          memset(USART_RX_BUF, 0, sizeof(USART_RX_BUF));
        ...
        if (GulPrintTimeCnt > 10) { 
            GulPrintTimeCnt = 0;
            switch (GnProtocalSelect)                   /* 通过状态值轮训来选择协议 */
            {
                case 0:
                    uartDrvPutBuf(USART2, UartCMDSpeedTxBuf, 3);    /* ask for speed frame */ 
                if (nProSelCnt++ > 3)               /* 连续发送5次均未收到回复则切换协议 */
                    {
                        nProSelCnt = 0;
                        GnProtocalSelect++;
                    }
                ...
                case 1:
                    uartDrvPutBuf(USART2, UartCMDSpeedTxBuf1, 3);    /* ask for speed frame */ 
                    if (nProSelCnt++ > 3)
                    {
                        nProSelCnt = 0;
                        GnProtocalSelect = 0;
...
//    TopDisp  =1;
    ...
    { 
        ...
        strcpy((char*)MenuItem[1] ,"2:Blower-贝丰 ");    //BlowerC60Test
